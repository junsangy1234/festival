"""API 키가 없을 때 Gemini 호출이 죽지 않고 규칙 원문·미표시로 폴백하는지 확인하는 최소 체크."""
from app import gemini_service
from app.gemini_service import estimate_confidence
...
def test_recommendation_falls_back_to_rule_text_without_api_key():
    ...
    result = gemini_service.expand_recommendation({"festivalName": "테스트 축제"}, recommendation)
...
def test_risk_severity_falls_back_to_rule_severity():
    ...
    result = gemini_service.judge_risk_severity({"festivalName": "테스트 축제"}, risk)
...
def test_briefing_is_hidden_without_api_key():
    ...
    result = gemini_service.build_briefing({"festivalName": "테스트 축제"})
...
def test_low_confidence_estimate_is_not_displayed():
    ...
    result = gemini_service.estimate_place_visitors(context, place)
